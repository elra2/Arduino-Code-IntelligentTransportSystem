...
// ---- Gripper Rotate  ----// Servo x, Pin x
      ...
      Serial.print("  under = ");
      Serial.println(SonarSensor(U_Trig, U_Echo)); 
      Serial.print("  AutoMode:");
      Serial.println(AutoMode); 
          
      if (AutoMode == 1){
      if (SonarSensor(U_Trig, U_Echo) > 10){   
      Serial.println("EDGE DETECTED");  
      StopTriTrack();
      DetachWheels();  
          }
     else if (SonarSensor(L_Trig, L_Echo) > 15 && SonarSensor(R_Trig, R_Echo) > 15){   
      Serial.println("SAFE");      
      TriTrackForward();
        } 
      ...
      TriTrackRight();   
          ...
          if(Xbox.getButtonClick(X,i))   {        //Unlock Wheels and employ manual mode
              AutoMode = 0;                                   // Detaches all servos - waiting for button to re-engage
              AttachWheels();
              }
